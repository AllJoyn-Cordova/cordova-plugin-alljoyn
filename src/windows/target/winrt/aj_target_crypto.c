...
#else // WINAPI_FAMILY_PARTITION(WINAPI_PARTITION_PC_APP)
	auto buffer = Windows::Security::Cryptography::CryptographicBuffer::GenerateRandom(len);
	auto reader = Windows::Storage::Streams::DataReader::FromBuffer(buffer);

	Platform::Array<uint8_t> data(reader->UnconsumedBufferLength);

	if (data.Length != 0)
	{
		reader->ReadBytes(Platform::ArrayReference<uint8_t>(rand, len));
	}
